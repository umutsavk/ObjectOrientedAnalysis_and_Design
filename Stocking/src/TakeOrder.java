...
public class TakeOrder {
	...
	public String[] employeeNames() {
		String[] temp = new String[Management.getManagement().getAdmin().getEmployees().size()];
		
		int i = 0;
		for (Employee emp : Management.getManagement().getAdmin().getEmployees()) {
			temp[i]= emp.getName();
			i++;
		}
		return temp;
	...
	public String[] tableNumbers() {
		String[] temp = new String[Management.getManagement().getTables().size()];
		int i = 0;
		for (Table t : Management.getManagement().getTables().values()) {
	...
	public TakeOrder(final JFrame superF) 
	{
		frame = new JFrame();
		...
		cbxEmployeesName.setModel(new DefaultComboBoxModel(employeeNames()));
		...
		cbxTableNumbers.setModel(new DefaultComboBoxModel(tableNumbers()));
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				to2 = new TakeOrder2(frame,cbxEmployeesName.getSelectedItem().toString(),Integer.parseInt(cbxTableNumbers.getSelectedItem().toString()));
